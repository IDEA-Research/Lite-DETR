...
class Transformer(nn.Module):
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        #########################################################
        # Beigin preparing tgt
        #########################################################   
        # if refpoint_embed is not None:
        #     tgt = tgt.transpose(0, 1)
        #     refpoint_embed = refpoint_embed.transpose(0, 1)
            # refpoint_embed = refpoint_embed.unsqueeze(1).repeat(1, bs, 1) # nq, bs, d_model
            # raw_nq = refpoint_embed.shape[0]
            # if self.learnable_tgt_init:
            #     tgt = self.tgt_embed.weight[:, None, :].repeat(1, bs, 1) # nq, bs, d_model
            # else:
            #     tgt = torch.zeros(raw_nq, bs, self.d_model, device=refpoint_embed.device)
            ...
            #     tgt_pat = self.patterns.weight[None, :, :].repeat_interleave(raw_nq, 1) # 1, n_q*n_pat, d_model
        ...
        if self.two_stage_type in ['standard', 'combine', 'enceachlayer', 'enclayer1']:
            ...
            # if self.rm_detach and 'enc_ref' in self.rm_detach:
            ...
            # if self.rm_detach and 'enc_tgt' in self.rm_detach:
            #     tgt = tgt_undetach
            # else:
...
class TransformerDecoder(nn.Module):
    ...
    def forward(self, tgt, memory,
        ...
        # return [
        #     torch.stack(intermediate).transpose(1, 2), # n_dec, bs, nq, d_model
        #     torch.stack(ref_points).transpose(1, 2), # n_dec+1, bs, nq, query_dim
        # ]
