...
class DeformableTransformer(nn.Module):
    ...
    def __init__(self, d_model=256, nhead=8, 
                 ...
                 dim_feedforward_enc=2048, dropout=0.0,
        ...
        if deformable_encoder:
            encoder_layer = DeformableTransformerEncoderLayer(d_model, dim_feedforward_enc,
                                                          dropout, activation,
                                                          num_feature_levels, nhead, enc_n_points,
                                                              add_channel_attention=add_channel_attention,
                                                              use_deformable_box_attn=use_deformable_box_attn,
                                                              box_attn_type=box_attn_type, use_pytorch_version=use_pytorch_version, value_proj_after=value_proj_after, deformable_use_checkpoint=deformable_use_checkpoint, same_loc=same_loc, proj_key=proj_key, key_aware=key_aware)
            # small linear project for high-resolution features
            if small_expand:
                encoder_layer_last = DeformableTransformerEncoderLayer(d_model, dim_feedforward_enc,
                                                          dropout, activation,
                                                          num_feature_levels, nhead, enc_n_points,
                                                              add_channel_attention=add_channel_attention,
                                                              use_deformable_box_attn=use_deformable_box_attn,
                                                          box_attn_type=box_attn_type, use_pytorch_version=use_pytorch_version,
                                                        value_proj_after=value_proj_after, enc_scale=enc_scale, small_expand=small_expand, deformable_use_checkpoint=deformable_use_checkpoint, same_loc=same_loc, proj_key=proj_key, key_aware=key_aware)

            else:
                encoder_layer_last = None
        else:
        ...
        self.encoder = TransformerEncoder(
            ...
            enc_scale=enc_scale,
            encoder_layer_last=encoder_layer_last,
            num_expansion=num_expansion,
        )

        # choose decoder layer type
        if deformable_decoder:
            decoder_layer = DeformableTransformerDecoderLayer(d_model, dim_feedforward_dec,
                                                          dropout, activation,
                                                          ...
                                                          module_seq=module_seq, use_pytorch_version=use_pytorch_version, value_proj_after=value_proj_after, key_aware=key_aware)
    ...
    def forward(self, srcs, masks, refpoint_embed, pos_embeds, tgt, attn_mask=None):
        ...
        #########################################################
        # import ipdb; ipdb.set_trace()
        src_tgt = src_flatten[:, level_start_index[4 - self.enc_scale]:]
        # pos_tgt = lvl_pos_embed_flatten[:, level_start_index[4 - self.enc_scale]:]
        memory, src_tgt, enc_intermediate_output, enc_intermediate_refpoints = self.encoder(
                src_flatten,
                src_tgt,
        ...
        if self.two_stage_type == 'standard':
            ...
            # gather boxes
            refpoint_embed_undetach = self.enc_out_bbox_embed(tgt_undetach) + init_box_proposal  # unsigmoid
...
class TransformerEncoder(nn.Module):
    ...
    def __init__(self, 
        ...
        self.enc_scale = enc_scale
        # prepare layers
        if encoder_layer_last is not None:
            if num_expansion==1:
                self.layers = nn.ModuleList([copy.deepcopy(encoder_layer) for i in range(num_layers-1)] + [encoder_layer_last])
            else:
                layers = []
                for i in range(num_expansion):
                    for j in range(int(self.num_layers/num_expansion)-1):
                        layers.append(copy.deepcopy(encoder_layer))
                    layers.append(copy.deepcopy(encoder_layer_last))
        ...
        #             [copy.deepcopy(encoder_layer) for i in range(num_layers - 1)] + [encoder_layer_last])
        #     else:
        #         self.layers = nn.ModuleList([copy.deepcopy(encoder_layer) for i in range(num_layers)])
    ...
    def forward(self, 
        ...
        pos_tgt = pos[:, level_start_index[4 - self.enc_scale]:]
        # intermediate_coord = []
        # main process
        for layer_id, layer in enumerate(self.layers):
            ...
            if not dropflag:
                # if self.deformable_encoder:
                # if self.num_expansion == 1:
                #     if layer_id == self.num_layers - 1:
                #         tgt = src
                #         output = layer(src=src, tgt=tgt, pos=pos, reference_points=reference_points,
                ...
                if (layer_id + 1) % (self.num_layers / self.num_expansion) == 0:
                    ...
                    output = layer(src=src, tgt=tgt, pos=pos, reference_points=reference_points,
                                   spatial_shapes=spatial_shapes, level_start_index=level_start_index,
                                   key_padding_mask=key_padding_mask)
                    tgt = output[:, level_start_index[4 - self.enc_scale]:]
                    src = output
                else:
                    output = layer(src=src, tgt=tgt, pos=pos_tgt, reference_points=reference_points_tgt, spatial_shapes=spatial_shapes, level_start_index=level_start_index, key_padding_mask=key_padding_mask)
                    tgt = output
                    src = torch.cat([src[:, :level_start_index[4 - self.enc_scale]], tgt], 1)
                # src[:, level_start_index[4 - self.enc_scale]:] = tgt
        ...
        return src, output, intermediate_output, intermediate_ref
...
class DeformableTransformerEncoderLayer(nn.Module):
    def __init__(self,
        ...
        if use_deformable_box_attn:
            ...
            self.self_attn = MSDeformAttn(d_model, n_levels, n_heads, n_points, use_pytorch_version=use_pytorch_version, value_proj_after=value_proj_after, proj_key=proj_key, deformable_use_checkpoint=deformable_use_checkpoint, same_loc=same_loc, key_aware=key_aware)
    ...
    def forward_ffn2(self, src):
        src2 = self.linear22(self.dropout22(self.activation2(self.linear12(src))))
        src = src + self.dropout32(src2)
        src = self.norm22(src)
        return src

    def forward(self, src, tgt, pos, reference_points, spatial_shapes, level_start_index, key_padding_mask=None):
        # self attention
        tgt2 = self.self_attn(self.with_pos_embed(tgt, pos), reference_points, src, spatial_shapes, level_start_index, key_padding_mask)
        ...
        if self.small_expand:
            tgt_3s = tgt[:, level_start_index[4 - self.enc_scale]:]
            tgt_4s = tgt[:, :level_start_index[4 - self.enc_scale]]
            tgt_4s = self.forward_ffn2(tgt_4s)
            tgt_3s = self.forward_ffn(tgt_3s)
            tgt = torch.cat([tgt_4s, tgt_3s], 1)
        else:
            tgt = self.forward_ffn(tgt)

        # channel attn
        if self.add_channel_attention:
            tgt = self.norm_channel(tgt + self.activ_channel(tgt))
...
class DeformableTransformerDecoderLayer(nn.Module):
    def __init__(self, d_model=256, d_ffn=1024,
        ...
        if use_deformable_box_attn:
            ...
            self.cross_attn = MSDeformAttn(d_model, n_levels, n_heads, n_points, use_pytorch_version=use_pytorch_version, value_proj_after=value_proj_after, proj_key=proj_key, key_aware=key_aware)
        self.dropout1 = nn.Dropout(dropout)
        self.norm1 = nn.LayerNorm(d_model)
...
def build_deformable_transformer(args):
    ...
    return DeformableTransformer(
        ...
        use_detached_boxes_dec_out=use_detached_boxes_dec_out,
        enc_scale=args.enc_scale,
        dim_feedforward_dec=args.dim_feedforward_dec,
        use_pytorch_version=args.use_pytorch_version,
        value_proj_after=args.value_proj_after,
        small_expand=args.small_expand,
        num_expansion=args.num_expansion,
        deformable_use_checkpoint=args.deformable_use_checkpoint,
        same_loc=args.same_loc,
        proj_key=args.proj_key,
        key_aware=args.key_aware,
    )
